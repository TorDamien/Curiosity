...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  T->largeur = l;
  T->hauteur = h;
  int p = dObst * 100; //proba exprimé en nombre entre 0 et 100
  for (int i = 0; i<l;i++){
    for (int j = 0; j<h;j++){
      if(rand() % 100 <= p ){  //Si dans la proba
        T->tab[i][j] = ROCHER; //met un rocher
        if(i== h/2 && j==l/2){ //mais pas à la case centrale
          T->tab[i][j] = LIBRE; 
        }
      }
      else{
        T->tab[i][j] = LIBRE;       //Sinon case LIBRE
      }
    }
  }
}
