...
resultat_deplacement avancer_envt(Environnement *envt) {
  ...
  /* Si le mod observateur est activé */
  if (envt->interrupteur == 1) {

    if ( (envt->a.etat_actuel) != M_DEVANT ) {
      gestion_erreur_observateur(envt->a.etat_actuel);

    }
    envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][AVANCER];
  }
...
void gauche_envt(Environnement *envt) { 
  if (envt->interrupteur == 1) {
    envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][GAUCHE];
  }
...
void droite_envt(Environnement *envt) { 
  if (envt->interrupteur == 1) {
    envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][DROITE];
  }
...
int mesure_envt(Environnement *envt, int d) {
  ...
  /* Partie observateur */
  if (envt->interrupteur == 1) {
    if ( (d == 1) && ((envt->t.tab[mx][my]) == LIBRE) ) { // Mesure devant libre
      envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][M_DEVANT_LIBRE];
    }
    else if ( (d == 3) && ((envt->t.tab[mx][my]) == LIBRE) ) { // Mesure droite libre
      envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][M_DROIT_LIBRE];
    }
    else if ( (d == 5) && ((envt->t.tab[mx][my]) == LIBRE) ) { // Mesure derrière libre
      envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][M_DERRIERE_LIBRE];
    }
    else if ( (d == 7) && ((envt->t.tab[mx][my]) == LIBRE) ) { // Mesure gauche libre
      envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][M_GAUCHE_LIBRE];
    }
    else {
      envt->a.etat_actuel = envt->a.transitions[envt->a.etat_actuel][M_AUTRE];
    }
  }
